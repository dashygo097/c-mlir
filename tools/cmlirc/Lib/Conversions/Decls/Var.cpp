...
auto CMLIRConverter::TraverseVarDecl(clang::VarDecl *decl) -> bool {
  ...
  if (clangType->isPointerType()) {
    ...
    if (decl->hasInit()) {
      ...
      if (!mlir::isa<mlir::LLVM::LLVMPointerType>(initValue.getType())) {
        initValue = utils::toPointer(builder, mlirLoc, initValue, ptrType);
      }

      if (!initValue) {
        llvm::WithColor::error()
            << "cmlirc: failed to initialize pointer variable: "
            << decl->getNameAsString() << "\n";
        return false;
      }

      mlir::LLVM::StoreOp::create(builder, mlirLoc, initValue,
                                  allocaOp.getResult());
    }
...

  {
    ...
    if (vat) {
      ...
      if (!elemType) {
        llvm::WithColor::error()
            << "cmlirc: failed to convert VLA element type for variable: "
            << decl->getNameAsString() << "\n";
        return false;
      }
  ...
  auto allocaOp = mlir::memref::AllocaOp::create(builder, mlirLoc, allocaType);
  symbolTable[decl] = allocaOp.getResult();
  ...
  if (decl->hasInit()) {
    ...
    if (auto *initList = mlir::dyn_cast<clang::InitListExpr>(init)) {
      ...
      if (initValue.getType() != mlirType) {
        if (mlir::Value casted =
                utils::toValue(builder, mlirLoc, initValue, mlirType)) {
          initValue = casted;
        }
      }
